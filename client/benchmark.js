...
var keypath = path.join(__dirname, 'wallet')
client.ccInit('Peer0', keypath, 'mychannel', 'socialmedia', '114.local:7051', '113.local:7050');
...
async function multibenchmark(){
    ...
    for(var i = 0; i < benchmarks; i++){
        ...
        var r = await benchmark(number, length , true, true);
...
async function benchmark(totalnumber, datalength, OCS=true, BCS =true){
    ...
    for (i = 0; i < totalnumber; i++){
        ...
        if(OCS){
            var HLargs = await client.StoreDataFS(new Buffer(value), String(i))
        }else{
            var HLargs = [ String(i),
            'eb2227ce2958d6dcc93f00b82c498b75',
            'file:///home/ubuntu/datastore',
        ...
        if(BCS){
        client.StoreDataHL(HLargs, checkIfSendDone).then((res) => {
