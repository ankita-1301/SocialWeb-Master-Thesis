...
installChaincode () {
	...
	VERSION=${3:-1.2}
        set -x
	peer chaincode install -n socialmedia -v ${VERSION} -l ${LANGUAGE} -p ${CC_SRC_PATH} >&log.txt
	res=$?
        set +x
	cat log.txt
...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
		peer chaincode instantiate -o orderer.ptunstad.no:7050 -C $CHANNEL_NAME -n socialmedia -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["addMetaData","2019-09-11 17:00:00","2019-09-11 18:00:00","QmPcb8mG3uiyDNWEerA6QaH3Kp5op7dS5gRDbGHbfeN9Mm","file:///home/ubuntu/datastore", "cfile"]}' -P "OR('Org1MSP.member','Org1MSP.member')" >&log.txt
		res=$?
                set +x
	else
                set -x
		peer chaincode instantiate -o orderer.ptunstad.no:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n socialmedia -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["init"]}' -P "OR	('Org1MSP.peer','Org2MSP.peer')" >&log.txt
		res=$?
                set +x
	fi
	cat log.txt
...
upgradeChaincode () {
    ...
    set -x
    peer chaincode upgrade -o orderer.ptunstad.no:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n socialmedia -v 2.0 -c '{"Args":["init"]}' -P "OR ('Org1MSP.peer','Org2MSP.peer','Org3MSP.peer')"
    res=$?
	  set +x
    cat log.txt
...
chaincodeQuery () {
  ...
  do
    ...
    set -x
    peer chaincode query -C $CHANNEL_NAME -n socialmedia -c '{"Args":["getFileLocation"]}' >&log.txt
    res=$?
    set +x
